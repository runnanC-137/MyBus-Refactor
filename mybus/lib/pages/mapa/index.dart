...
import 'package:mybus/classes/PontoBus.dart';
import 'package:mybus/classes/Transporte.dart';
...
class _MapaState extends State<Mapa> with WidgetsBindingObserver {
  ...
  late Symbol _symbolSelecionado;
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      if (_retornoBusAtivo)
        showOkAlertDialog(
            ...
            title: "Atenção",
            message:
                "Por motivos de segurança, só compartilhamos sua localização caso esteja no aplicativo!");
      if (_retornoBusAtivo) {
        criarTransporte();
      ...
      if (_busAtivo) deletarTransporte();
    }
  ...
  void _onSymbolTapped(Symbol symbol) {
    if (_symbolSelecionado != null) {
      mapController.updateSymbol(
          _symbolSelecionado, const SymbolOptions(iconSize: 1.2));
    }

    _symbolSelecionado = symbol;

    mapController.updateSymbol(
        _symbolSelecionado, const SymbolOptions(iconSize: 1.5));

    if (_symbolSelecionado.options.iconImage != "") {
      //Removendo os symbol pré carregados que não tem icone
      if (_symbolSelecionado.options.iconImage == "bus") {
        //Foi selecionado o ponto de ônibus
        String? _id = _symbolSelecionado.options.textTransform;
        janelaPontoBusMapBox(_id!);
      } else {
        //Selecionado algum transporte
        String? _id = _symbolSelecionado.options.textTransform;
        print(_id);
        ...
        if (dados.type == DocumentChangeType.added) {
          ...
          Symbol symbol = await mapController.addSymbol(
            SymbolOptions(
              ...
              textTransform:
                  _id, //Isso é um "atalho", evita muito código de busca para encontrar o dado, não retirar a menos que encontre um método mais otimizado
              textAnchor: 'top',
              iconImage: 'bus',
              iconSize: 1.2,
              iconAnchor: 'bottom',
              geometry: LatLng(_dadosPontoBus[_id]['localAtual'].latitude,
          ...
          await mapController.updateSymbol(_symbolPontoBus[_id],
              SymbolOptions(textField: _dadosPontoBus[_id]['nome']));
          print("Dado atualizado! ${_dadosPontoBus[_id]}");
        } else if (dados.type == DocumentChangeType.removed) {
          //Removido do firebase
          String _id = dados.doc.id;
          if (_symbolSelecionado == _symbolPontoBus[_id]) {
            // Caso eu esteja deletando o symbol selecionado
            /* _symbolSelecionado = null; */
          }
      ...
      snapshot.docChanges.forEach((dados) async {
        if (dados.type == DocumentChangeType.added) {
          if ((Timestamp.now().seconds -
                  dados.doc.data()?['ultimaAtualizacao']) <=
              60) {
            //Adicionado ao firebase(Quando abre a requisição, vai ser carregado os dados como se tivessem sido adicionados ao firebase)
            String _id = dados.doc.id;
          ...
          * */
            Symbol symbol = await mapController.addSymbol(
              SymbolOptions(
                  geometry: LatLng(_dadosTransporte[_id]['localAtual'].latitude,
          ...
          //Removido do firebase
          String _id = dados.doc.id;
          if (_symbolSelecionado == _symbolTransporte[_id]) {
            //Caso eu esteja deletando o symbol selecionado
            // _symbolSelecionado = null;
          }
  ...
  void getPermissaoLocalizacao() async {
    ...
    if (!localAtivo) {
      ...
      showOkAlertDialog(
          context: context,
          title: "Atenção",
          message: "Seu GPS está desligado");
    }
    ...
    if (permissao == LocationPermission.denied) {
      ...
      if (permissao == LocationPermission.deniedForever) {
        print(
            "Permissão Negada Permanentemente! O usuário tem que ir nas configurações para alterar...");
        permissaoLocal = false;
        showOkAlertDialog(
            context: context,
            title: "Atenção",
            message:
                "O Aplicativo precisa da permissão para funcionar corretamente, por favor entre nas configurações e dê a permissão!");
      }
  ...
  void minhaLocalizacao(bool listen) async {
    try {
      if (listen) {
        StreamSubscription<
            Position> positionStream = Geolocator.getPositionStream(
                //Tem que lembrar que o banco de dados é gratuito, ou seja, tem limite de requisição, por isso tem que restringir!
          ...
          if (_position == null) {
            //teoricamente, só vai entrar na hora que abrir
            setState(() {
              _position = posicao;
              mapboxLocalizacao = true;
            });
            if (_busAtivo && _position != null && !_modoEspera)
              atualizarTransporte(); //Caso esteja compartilhando a localização
            if (_busAtivo && _position != null && _modoEspera)
              criarTransporte(); //Tentando sair do modo de espera
            if (_rotaAtiva && _rotaGerada != [])
              calculaTimePonto(
                  _rotaGerada); //Atualiza o tempo até a parada de ônibus
          } else {
            _position = posicao;
            if (_position == null) {
              //Caso ocorra uma falha, desativa a localização
              setState(() {
  ...
  void centralizarPosicao() {
    try {
      mapController.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(
          target: LatLng(_position.latitude, _position.longitude), zoom: 13)));
    } catch (error) {
      print(error);
    }
  }
                        ...
                        children: [
                          ...
                          Expanded(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              label: Text(_btnJanelaTransporteConfirmar),
                              onPressed: () {
                                if (_keyFormTransporte.currentState!
                                    .validate()) {
                                  //Criar o transporte
                                  criarTransporte();
                                  Navigator.pop(context);
  ...
  void janelaTransporteMapBox(String _id) {
    //Janela que aparece ao clicar no icone
  ...
  void janelaPontoBusMapBox(String _id) {
    //Janela que aparece ao clicar no icone
                            ...
                            child: ElevatedButton.icon(
                              ...
                              onPressed: () {
                                if (_keyFormPontoBus.currentState!.validate()) {
                                  ...
                                  if (symbolDentroRaio(
                                      _dadosPontoBus[_id]['localAtual'], 50)) {
                                    //Só pode deletar o ponto, caso esteja próximo dele, ou seja, teoricamente você está confirmando que ele não existe
                                    deletarPontoBus(_id);
                                    Navigator.pop(context);
                                  } else {
                                    showOkAlertDialog(
                                        ...
                                        title: "Atenção",
                                        message:
                                            "Só pode deletar ponto de ônibus caso esteja próximo, dessa forma você pode confirmar que ele não existe!");
                                  }
                                }
                              },
                              style: ElevatedButton.styleFrom(
  ...
  void criarTransporte() {
    ...
    GeoPoint posicaoBus = (_idBusProximo != "")
        ? _dadosTransporte[_idBusProximo]['localAtual']
        : const GeoPoint(0, 0); //0,0 fica no meio do mar, não vai ter problema
    if (!symbolDentroRaio(posicaoBus, 10)) {
      //Só cria o transporte caso não tenha nenhum a 10m, assim evita várias pessoas compartilhar o mesmo transporte
      Transporte transporte = Transporte(
          getNome(_tipoSelecionado),
          _tipoSelecionado,
          _destinoTransporte.text,
          GeoPoint(_position.latitude, _position.longitude));
      FirebaseFirestore firebase = FirebaseFirestore.instance;
      firebase
          .collection("Transporte")
          .doc(_id)
          .set(transporte.toMap())
          .then((value) {
        print("Transporte Criado!");
        final snackBar = const SnackBar(
          backgroundColor: Colors.green,
          behavior: SnackBarBehavior.floating,
          content: Text("Transporte criado!"),
        );
        setState(() {
        ...
        final snackBar = const SnackBar(
          ...
          behavior: SnackBarBehavior.floating,
          content: Text("Ops... Houve um erro!"),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      });
    } else {
      //Entra em modo de espera, até sair
      print("Transporte entrou em modo de espera!");
      final snackBar = const SnackBar(
        backgroundColor: Colors.amber,
        behavior: SnackBarBehavior.floating,
        content: Text("Você entrou em modo de espera!"),
      );
      setState(() {
        _modoEspera =
            true; //Quando distanciar irá compartilhar automaticamente, quando ficar false
        _busAtivo = true;
    ...
    if (!_modoEspera) {
      ...
      firebase.collection("Transporte").doc(_id).delete().then((value) {
        ...
        final snackBar = const SnackBar(
          backgroundColor: Colors.amber,
          behavior: SnackBarBehavior.floating,
          content: Text("Transporte cancelado!"),
        );
        setState(() {
          _busAtivo = false;
  ...
  void criarPontoBus() {
    minhaLocalizacao(false);
    PontoBus pontoBus = PontoBus(_nomePontoBus.text, _descricaoPontoBus.text,
        GeoPoint(_position.latitude, _position.longitude));
    FirebaseFirestore firebase = FirebaseFirestore.instance;
  ...
  String symbolMaisProximo(Map<String, dynamic> listaDados) {
    ...
    listaDados.forEach((_id, dado) {
      double novaDistacia = Geolocator.distanceBetween(
          minhaPosicao.latitude,
          minhaPosicao.longitude,
          dado['localAtual'].latitude,
          dado['localAtual'].longitude);
  ...
  bool symbolDentroRaio(GeoPoint symbolPosicao, double raio) {
    minhaLocalizacao(false);
    Position minhaPosicao = _position;
    double distanciaSymbol = Geolocator.distanceBetween(
        minhaPosicao.latitude,
        minhaPosicao.longitude,
        symbolPosicao.latitude,
        symbolPosicao.longitude);
  ...
  void buscarPontoBus() async {
    try {
      ...
      if (idPontoBus != "") {
        //Ou seja, encontrou uma ponto de bus
        ...
        const String access_token =
            'pk.eyJ1IjoibXlidXNwcm9qZXRvIiwiYSI6ImNrbnV4ZTR6dzBldDMycmxkbTdsZ3JleWEifQ.tEVYvy6Ya9q2KOCndaQvXw';
        ...
        if (valor['code'] != "InvalidInput") {
          ...
          for (int i = 0; i < rotaAUX.length; i++) {
            for (int j = 0; j < rotaAUX[i].length; j++) {
              GeoPoint aux = new GeoPoint(
                  rotaAUX[i][j]['location'][1], rotaAUX[i][j]['location'][0]);
              pontosRota.add(aux);
            }
          }
          if (pontosRota.isNotEmpty) {
            ...
            await mapController
              ...
              if (minhaPosicao.latitude <= symbolPosicao.latitude) {
                northeast =
                    new LatLng(symbolPosicao.latitude, symbolPosicao.longitude);
                southwest =
                    new LatLng(minhaPosicao.latitude, minhaPosicao.longitude);
              } else {
                northeast =
                    new LatLng(minhaPosicao.latitude, minhaPosicao.longitude);
                southwest =
            ...
            showOkAlertDialog(
                context: context,
                title: "Atenção",
                message: "Ocorreu um erro: Rota gerada é vazia!");
          }
        } else {
          showOkAlertDialog(
              context: context,
              title: "Atenção",
              message:
                  "Não existe caminho até o ponto mais próximo ou é muito longe!");
        }
      } else {
        showOkAlertDialog(
            context: context,
            title: "Atenção",
            message: "Não existe pontos de ônibus no mapa!");
      }
    } catch (error) {
      print(error);
    }
  ...
  void calculaTimePonto(List<GeoPoint> rota) {
    ...
    for (int i = 0; i < rota.length; i++) {
      //Encontra o ponto mais proxima na rota gerada, desse modo eu posso retirar o restante que já passou
      double novaDistancia = Geolocator.distanceBetween(_minhaPosicao.latitude,
          _minhaPosicao.longitude, rota[i].latitude, rota[i].longitude);
    ...
    if (valorProximo != 0) {
      if ((valorProximo + 1) < rota.length) {
        //aqui é adicionado + 1, pois quando o usuario passar do ponto, este ponto ainda vai continuar mais perto do que o próximo
        for (int i = (valorProximo + 1); i < rota.length; i++) {
          novaRota.add(rota[i]);
        }
      } else {
        if ((valorProximo + 1) ==
            rota.length) //Significa que o ponto proximo é o ultimo
    ...
    distanciaTotal += Geolocator.distanceBetween(
        _minhaPosicao.latitude,
        _minhaPosicao.longitude,
        novaRota[0].latitude,
        novaRota[0]
            .longitude); //Distância do usuario até o ponto mais proximo, OBS: já foi feito o ajuste no array, descartando os pontos passados
    for (int i = 0; (i + 1) < novaRota.length; i++) {
      distanciaTotal += Geolocator.distanceBetween(
          novaRota[i].latitude,
          novaRota[i].longitude,
          novaRota[i + 1].latitude,
          novaRota[i + 1].longitude);
    }
    double velocidade = _minhaPosicao
        .speed; //Recebe em M/s, porém não funfa em todos os dispositivos, nesse caso aparece como 0
    if (velocidade < 0.3) {
      //Pessoa está praticamente parada
      if (distanciaTotal >= 1000) {
        ...
        setState(() {
          _txtTimeBus =
              "Eu → Ponto: ${distanciaTotal.toStringAsFixed(1)} metros";
        });
      }
    } else {
          ...
          children: [
            ...
            Expanded(
              child: MapboxMap(
                ...
                styleString:
                    "mapbox://styles/mybusprojeto/cklsrgqne1qoo17qlj57b6fwv",
